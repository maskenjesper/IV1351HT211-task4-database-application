...
WITH given_lessons_in_selected_year as (
    ...
    WHERE extract (year FROM appointed_time) = 2021 AND executed = true
     ...
     lessons_pivoted as (
         SELECT month,
                round(avg(COALESCE(CASE WHEN lesson_type = 'individual_lesson' THEN count END,0)), 0) AS individual,
                round(avg(COALESCE(CASE WHEN lesson_type = 'group_lesson' THEN count END,0)), 0) AS group,
                round(avg(COALESCE(CASE WHEN lesson_type = 'ensamble' THEN count END,0)), 0) AS ensamble
